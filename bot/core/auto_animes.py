...
from os import path as ospath, system, remove as osremove
        ...
        if force or (not (ani_data := await db.getAnime(ani_id)) \
            ...
            for attempt in range(3):
                ...
                if dl and ospath.exists(dl):
                    break
                # cleanup possible broken part files
                if dl and ospath.exists(dl):
                    try:
                        osremove(dl)
                    except Exception:
                        pass
                await rep.report(f"File Download Failed, Retrying {attempt+1}/3", "warning")
                await asleep(3)
            ...
            for qual in Var.QUALS:
                ...
                if post_msg:
                    label = btn_formatter.get(qual, qual)
                    if len(btns) != 0 and len(btns[-1]) == 1:
                        btns[-1].insert(
                            1,
                            InlineKeyboardButton(f"{label} - {convertBytes(msg.document.file_size)}", url=link)
                        )
                    else:
                        btns.append([
                            InlineKeyboardButton(f"{label} - {convertBytes(msg.document.file_size)}", url=link)
