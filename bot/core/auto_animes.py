...
async def get_animes(name, torrent, force=False):
    try:
        ...
        if not force and qual_data and all(
            qual_data.get(q, {}).get("uploaded") for q in Var.QUALS
        ):
            return
        ...
        for attempt in range(3):
            ...
            if dl and ospath.exists(dl):
                break
            await rep.report(
                f"Download failed. Retrying ({attempt+1}/3)...", "warning"
            )
            await asyncio.sleep(5)

        if not dl or not ospath.exists(dl):
            await rep.report("File Download Incomplete after retries, Skipping", "error")
            try:
        ...
        for qual in Var.QUALS:
            ...
            try:
                uploaded_msg = await TgUploader(
                    stat_msg, ani_id, ep_no, qual
                ).upload(out_path, qual)
            except Exception as e:
...
async def start_pm_handler(client, message):
    ...
    if len(message.command) < 2:
        await message.reply("Welcome! Use the channel buttons to get files.")
        return
    ...
    if not already and file_info.get("msg_id"):
        await send_file_pm(user_id, ani_id, ep_no, qual, file_info["msg_id"])
    else:
        website = getattr(Var, "WEBSITE", None) or getattr(Var, "WEBSITE_URL", None)
        if website:
            await message.reply(f"🔗 Visit website for re-download:\n{website}")
        else:
            await message.reply("⏳ File is being prepared. Please try again later.")


# ----------------- Send File PM -----------------
async def send_file_pm(user_id, ani_id, ep_no, qual, msg_id):
    try:
