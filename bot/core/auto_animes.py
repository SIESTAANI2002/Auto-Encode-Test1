        ...
        if force or (not (ani_data := await db.getAnime(ani_id)) \
            ...
            for attempt in range(1, retries + 1):
                ...
                if dl and ospath.exists(dl):
                    ...
                    await asleep(60)  # wait 1 min before retry

            if not dl or not ospath.exists(dl):
                await rep.report(f"❌ File Download Incomplete after {retries} attempts, Skipped {name}", "error")
            ...
            for qual in Var.QUALS:
                ...
                if post_msg:
                    if len(btns) != 0 and len(btns[-1]) == 1:
                        btns[-1].insert(1, InlineKeyboardButton(f"{btn_formatter.get(qual, qual)} - {convertBytes(msg.document.file_size)}", url=link))
                    else:
                        btns.append([InlineKeyboardButton(f"{btn_formatter.get(qual, qual)} - {convertBytes(msg.document.file_size)}", url=link)])
